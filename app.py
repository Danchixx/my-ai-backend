...
def generate_identification_prompt(file_info, content=None, num_questions=10):
    """Generate identification-type questions (fill-in-the-blank/definition type)"""
    filename = file_info['filename']
    extension = file_info['extension']
    size_mb = file_info['size_mb']
    
    if content and content.strip():
        prompt = f"""Based on the content of this {extension.upper()} file, create {num_questions} identification-type questions for students.
...
IDENTIFICATION QUESTION FORMAT:
These are definition/fill-in-the-blank style questions where students identify key terms, concepts, or definitions.

Please create exactly {num_questions} identification questions in this EXACT format:

QUESTION 1: This is the process by which plants use sunlight to convert carbon dioxide and water into glucose.


QUESTION 2: ______________________ is a gas produced as a byproduct when organisms break down glucose for energy without oxygen.


QUESTION 3: [Your question text here]


IMPORTANT FORMATTING RULES:
- Format: "QUESTION X: [Question text]" (blank line after)
- MUST leave blank lines between question sections
- Make questions test important vocabulary and key concepts from the material
- Provide enough context in the Definition so students can reasonably identify the answer
- Vary difficulty levels (basic recall, comprehension, application)
...
Continue with this exact format for all {num_questions} questions. Each question MUST have a Definition section with blank lines separating them."""
    else:
        prompt = f"""I have a {extension.upper()} file named "{filename}" ({size_mb} MB) that couldn't be read properly. 
Please create {num_questions} general identification questions about typical {extension.upper()} document content using the identification format specified above."""
    
    return prompt

def generate_summary_prompt(file_info, content=None):
    """Generate a detailed, student-oriented lesson review prompt"""
    filename = file_info['filename']
    extension = file_info['extension']
    size_mb = file_info['size_mb']

    if content and content.strip():
        prompt = f"""You are an educational assistant helping a student review and deeply understand this {extension.upper()} file.
...
Your task is to create a detailed **lesson review** that helps the student study and retain key knowledge.  
Organize your output into the following sections:

1. Lesson Overview
   - Summarize what the lesson or material is about.
   - State the main topic and its importance or relevance.

2. Key Concepts and Definitions
   - List and explain all essential terms, formulas, or concepts.
   - Give simple explanations and real-life or classroom examples.

3. Main Ideas and Supporting Details
   - Break down each major point or section in the lesson.
   - Highlight connections between ideas and how they build understanding.

4. Step-by-Step Explanations (if applicable)
   - If the lesson involves processes, calculations, or reasoning steps, explain them clearly.

5. Important Takeaways
   - List the top 3–5 things the student must remember.
   - Emphasize definitions, formulas, or ideas often tested or used in practice.

6. Application and Examples
   - Provide short, practical examples or scenarios where the lesson applies.
   - Explain how these examples reinforce understanding.

7. Review Questions
   - Create a few short questions or prompts to help the student test comprehension.

8. Summary Conclusion
   - Restate the main idea and why it matters.
   - Offer advice on how to remember or apply the lesson effectively.

Output guidelines:
- Use clear, organized sections with headers.
- Use simple, student-friendly language.
- Avoid vague summaries; focus on helping the student learn and recall key information.
"""
    else:
        prompt = f"""I have a {extension.upper()} file named "{filename}" ({size_mb} MB) that could not be read.

Please describe generally what lessons or information {extension.upper()} files often contain,
and how a student might review or study them effectively. Include:
- Common learning objectives
- Usual structure of the material
- Study tips and review techniques relevant to this type of file
"""
    return prompt


def generate_questions_prompt(file_info, content=None, num_questions=10):
    """Generate multiple-choice questions for pass rate prediction"""
    filename = file_info['filename']
    extension = file_info['extension']
    size_mb = file_info['size_mb']
...
@app.route('/generate-identification-questions', methods=['POST'])
def generate_identification_questions():
    """Generate identification-type questions (for openAI.php)"""
    try:
        if 'file' not in request.files:
            return jsonify({"error": "No file part"}), 400
        ...
        if file and allowed_file(file.filename):
            ...
            content = extract_text_content(file_path, file_extension)
            identification_prompt = generate_identification_prompt(file_info, content, num_questions)
            ...
            return chat_with_ai(identification_prompt)
...
def generate_questions():
    """Generate multiple-choice questions (for pass_rate_prediction.php)"""
    try:
        if 'file' not in request.files:
            return jsonify({"error": "No file part"}), 400
...
def health_check():
    return jsonify({
        ...
        "features": [
            ...
            "Identification question generation",
